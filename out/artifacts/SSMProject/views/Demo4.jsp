...
<head>
    ...
    <link rel="stylesheet" type="text/css"
          href="${pageContext.request.contextPath}/js/demo/demo.css">
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery.min.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery.easyui.min.js"></script>
    <script type="text/javascript"
            src="${pageContext.request.contextPath}/js/locale/easyui-lang-zh_CN.js"></script>
    <script type="text/javascript"
            src="js/jquery.ztree.all-3.5.min.js"></script>
</head>
    ...
    <div id="tt" class="easyui-tabs"
        ...
        <div title="学生信息" data-options="iconCls:'icon-reload'"
            ...
            <div style="padding:5px;">
                <c:forEach items="${permissions}" var="ps">
                    <c:if test="${ps.p_id==7}">
                        <a id="addStu" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add'"
                           onclick="${ps.p_url}">${ps.p_title}</a>
                    </c:if>
                    <c:if test="${ps.p_id==8}">
                        <a id="deleteStu" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove'"
                           onclick="${ps.p_url}">${ps.p_title}</a>
                    </c:if>
                    <c:if test="${ps.p_id==9}">
                        <a id="editStu" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-edit'"
                           onclick="${ps.p_url}">${ps.p_title}</a>
                    </c:if>
                </c:forEach>
                <a id="loadStu" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-reload'"
                   onclick="tableload()">刷新表格</a>
            </div>
            <div style="margin: 20px 0"></div>
            <p>
...
<div id="rtt">
    <c:forEach items="${permissions}" var="ps">
        ...
        <c:if test="${ps.p_id==18}">
            <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-man',plain:true"
               onclick="${ps.p_url}">${ps.p_title}</a>
        </c:if>
    </c:forEach>
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (row.parent_id == node.id) {
                    var child = {
                        ...
                        text: row.p_title,
                        checked: row.checked,
                        attributes: {url: row.p_url, parent: row.parent_id}
                    };
                    if (node.children) {
